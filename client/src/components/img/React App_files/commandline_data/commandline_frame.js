...
(() => {
  ...
  function minimalKeyFromKeyboardEvent(keyEvent) {
    ...
    if (get2("keyboardlayoutforce") === "true") {
    ...
    if (get2("usekeytranslatemap") === "true") {
  ...
  function updateBaseLayout() {
    KEYCODETRANSLATEMAP = mergeRight_default(
      keyboardlayouts[get2("keyboardlayoutbase")],
      get2("keyboardlayoutoverrides")
    );
  }
...
statistics, so somewhere around 10k samples.`, new StringType(false, false), false)],
      ...
      ["get", new SymbolMetadata("Get the value of the key target.\n\nIf the user has not specified a key, use the corresponding key from\ndefaults, if one exists, else undefined.", new FunctionType([new UnionType([
        ...
        new TypeReferenceType("... 88 more ...", [], false, false),
...
When adding updaters, don't forget to set("configversion", newversionnumber)!`, new FunctionType([], new TypeReferenceType("Promise", [new BooleanType(false, false)], false, false), false, false), true)],
  ...
  async function getSortedTabs(forceSort) {
    const sortAlg = forceSort != null ? forceSort : get2("tabsort");
    const comp = sortAlg === "mru" ? (a, b) => +a.active || -b.active || b.lastAccessed - a.lastAccessed : (a, b) => a.index - b.index;
    const hiddenVal = get2("tabshowhidden") === "true" ? void 0 : false;
  ...
  // src/completions/Autocmd.ts
  var AutocmdCompletionOption = class extends CompletionOptionHTML {
    constructor(displayValue, description, value) {
      super();
  ...
  var AutocmdCompletionSource = class extends CompletionSourceFuse {
    constructor(_parent) {
      super(
      ...
      this.updateOptions();
      this.shouldSetStateFromScore = get2("completions", "Autocmd", "autoselect") === "true";
      this._parent.appendChild(this.node);
    }
    ...
    async updateOptions(exstr = "") {
      this.lastExstr = exstr;
      const [prefix, rest2] = this.splitOnPrefix(exstr);
      const args = rest2 ? rest2.split(/\s+/) : [];
      if (prefix) {
        if (this.state === "hidden") {
          this.state = "normal";
        }
      ...
      const defined_autocmds = get2("autocmds");
      ...
      if (args.length < 2) {
        const all_autocmds = await message(
          "excmd_background",
          "getAutocmdEvents"
        );
        let autocmds_to_suggest;
        if (filter_defined_autocmds) {
          const defined_events = new Set(Object.keys(defined_autocmds));
          autocmds_to_suggest = all_autocmds.filter(
            (s) => defined_events.has(s)
          );
        } else {
          autocmds_to_suggest = all_autocmds;
        }
        if (args.length > 0) {
          autocmds_to_suggest = autocmds_to_suggest.filter(
            (s) => s.startsWith(args[0])
          );
        }
        this.options = autocmds_to_suggest.sort((a, b) => a.localeCompare(b)).map((event) => {
          let value = event;
        ...
        const event = args[0];
        const pat = args[1];
        const existing_patterns = Object.entries(defined_autocmds).filter(([e, _]) => !filter_defined_autocmds || e == event).map(([_, v]) => Object.entries(v)).flat();
        this.options = existing_patterns.filter(([pattern, _]) => pattern.startsWith(pat)).sort((a, b) => a[0].localeCompare(b[0])).map(([pattern, command]) => {
          let description = "";
          if (is_autocmddelete) {
            description = command;
          }
  ...
  async function getSearchUrls(query2) {
    const suconf = get2("searchurls");
    const searchUrls = [];
    for (const prop in suconf) {
      if (Object.prototype.hasOwnProperty.call(suconf, prop) && prop.startsWith(query2)) {
  ...
  async function getCombinedHistoryBmarks(query2) {
    const [history2, bookmarks, searchUrls] = await Promise.all([
      getHistory(query2),
      getBookmarks(query2),
    ...
    const score = (x) => (x.history ? frecency(x.history) : 0) - (x.bmark ? get2("bmarkweight") : 0) - (x.search ? get2("searchurlweight") : 0);
  ...
  var BmarkCompletionSource = class extends CompletionSourceFuse {
    ...
    async filter(exstr) {
      ...
      this.lastExstr = [prefix, query2].join(" ");
      return this.updateChain();
    }
    setStateFromScore(scoredOpts) {
      super.setStateFromScore(scoredOpts, this.shouldSetStateFromScore);
  ...
  var HistoryCompletionOption = class extends CompletionOptionHTML {
    constructor(page, options) {
      ...
      this.fuseKeys.push(preplain, page.title, page.url);
  ...
  var _HistoryCompletionSource = class extends CompletionSourceFuse {
    constructor(_parent) {
      super(
        ["open", "tabopen", "winopen"],
        "HistoryCompletionSource",
        _HistoryCompletionSource.DEFAULT_SECTION_HEADER
      );
      this._parent = _parent;
      this._parent.appendChild(this.node);
    }
    async filter(exstr) {
      ...
      const tokens = query2.split(" ");
      if (tokens.length > 1 || query2.endsWith(" ")) {
        const match = (await getSearchUrls(tokens[0])).find(
          (su) => su.title === tokens[0]
        );
        if (match !== void 0) {
          query2 = tokens.slice(1).join(" ");
          this.updateSectionHeader("Search " + match.title, headerPostfix);
          query2 = match.url.split("%s").join(" ") + " " + query2;
        }
      }
    ...
    updateSectionHeader(newTitle, postfix) {
      if (postfix.length > 0) {
  ...
  var HistoryCompletionSource = _HistoryCompletionSource;
  HistoryCompletionSource.DEFAULT_SECTION_HEADER = "History and bookmarks";
  ...
  function computeDate(session) {
    let howLong = Math.round(
      (/* @__PURE__ */ new Date() - session.lastModified) / 1e3
    );
    let qualifier = "s";
  ...
  var BufferCompletionOption = class extends CompletionOptionHTML {
    constructor(value, tab, isAlternative = false, container, tabIndex) {
      super();
      this.value = value;
  ...
  var BufferCompletionSource = class extends CompletionSourceFuse {
    ...
    TabscoredOptionsStartsWithN(n, options) {
      ...
      for (const [index2, option] of enumerate(options)) {
        if ((option.tabIndex + 1).toString().startsWith(nstr)) {
          res.push({
            index: index2,
            // index is not tabIndex, changed by score
            option,
            score: 0
          });
        }
    ...
    async fillOptions(prefix) {
      const altTab = await prevActiveTab();
      const forceSort = prefix === "tabmove" ? "default" : void 0;
      ...
      for (const [index2, tab] of tabs.entries()) {
  ...
  var TabAllCompletionOption = class extends CompletionOptionHTML {
    constructor(value, tab, isAlternative, isCurrent, winindex, container, incognito, tgroupname) {
      super();
      this.value = value;
      this.fuseKeys = [];
      ...
      tgroupname = tgroupname === void 0 ? "" : tgroupname;
      this.fuseKeys.push(pre);
      this.fuseKeys.push(preplain);
  ...
  var TabGroupCompletionOption = class extends CompletionOptionHTML {
    constructor(group, tabCount, current, alternate, audible, urls) {
      super();
      this.fuseKeys = [];
      this.value = group;
      let preplain = "";
    ...
    async updateOptions(exstr = "") {
      ...
      this.options = await Promise.all(
        groups.map(async (group) => {
          ...
          const o2 = new TabGroupCompletionOption(
            group,
            tabs.length,
            group === currentGroup,
